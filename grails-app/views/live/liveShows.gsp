...
<div class="inside-page-container">
    ...
    <g:set var="evenNumberOfFutureLiveShows" value="${futureLiveShows.size() % 2}"/>
    <g:each in="${futureLiveShows}" var="liveShow" status="index">
        <div class="d-flex flex-column flex-md-row justify-content-center align-items-start liveShowEntry ${index % 2 == 0? 'borderClass':''} ${((futureLiveShows.size() == index + 1) && (index % 2 != 0))? 'last-Element':''}">
    ...
    <g:each in="${pastLiveShows}" var="liveShow" status="index">
        <div class="d-flex flex-column flex-md-row justify-content-center align-items-start liveShowEntry ${index % 2 == 0? 'borderClass':''} ${((pastLiveShows.size() == index + 1) && (index % 2 != 0))? 'last-Element':''}">
            <div class="liveShow-date"><g:formatDate date="${liveShow.date}" format="dd.MMMM.yyyy"/></div>
            <div class="liveShow-venue">${liveShow.venue}</div>
            <div class="liveShow-city d-flex justify-content-between">${liveShow.city}
                <div class="d-flex flex-row">
